...
HEIGHT = 500
BALL_CRASH_THRES = 60
BALL_CRASH_THRES_B = 60
TOP_CRASH_THRES = 50
BUTTOM_CRASH_THRES = 50
LEFT_CRASH_THRES = 50
RIGHT_CRASH_THRES = 50
START_VALOCITY_THRES = 10
SHOOTING_DISTANCE_THRES = 1000
START_TRACKING_THRES = 3
MIN_BALL_AREA = 200
MAX_BALL_AREA = 500
ACT_VALOCITY_THRES = 2
...
class Event_checker():
    ...
    def read_simulation_result(self, path):
        with open(path, 'r') as f:
            data = json.load(f)
        red_x = np.uint(np.array(data["rBallPositions"][0::2])/2844*1000)
        red_y = np.uint(np.array(data["rBallPositions"][1::2])/1422*500)
        ...
        yellow_x = np.uint(np.array(data["yBallPositions"][0::2]) / 2844 * 1000)
        yellow_y = np.uint(np.array(data["yBallPositions"][1::2]) / 1422 * 500)
        ...
        white_x = np.uint(np.array(data["wBallPositions"][0::2]) / 2844 * 1000)
        white_y = np.uint(np.array(data["wBallPositions"][1::2]) / 1422 * 500)
    ...
    def capture_shoot(self, img, img_color):
        ...
        cv2.circle(img_binary, (int(self.ball_path_list['red'][-1][0]), int(self.ball_path_list['red'][-1][1])), int(self.ball_radius), 0, -1)
        cv2.circle(img_binary, (int(self.ball_path_list['red'][-1][0]), int(self.ball_path_list['red'][-1][1])), int(self.ball_radius), 0, 3)

        cv2.circle(img_binary, (int(self.ball_path_list['yellow'][-1][0]), int(self.ball_path_list['yellow'][-1][1])), int(self.ball_radius), 0, -1)
        cv2.circle(img_binary, (int(self.ball_path_list['yellow'][-1][0]), int(self.ball_path_list['yellow'][-1][1])), int(self.ball_radius), 0, 3)

        cv2.circle(img_binary, (int(self.ball_path_list['white'][-1][0]), int(self.ball_path_list['white'][-1][1])), int(self.ball_radius), 0, -1)
        cv2.circle(img_binary, (int(self.ball_path_list['white'][-1][0]), int(self.ball_path_list['white'][-1][1])), int(self.ball_radius), 0, 3)

        if self.init_ball_pos != None:
            cv2.circle(img_binary,
                   (self.init_ball_pos[0]['x'], self.init_ball_pos[0]['y']),
                   self.init_ball_pos[0]['radius'], 0, -1)
            cv2.circle(img_binary,
                   (self.init_ball_pos[0]['x'], self.init_ball_pos[0]['y']),
                   self.init_ball_pos[0]['radius'], 0, 3)

            cv2.circle(img_binary,
                       (self.init_ball_pos[1]['x'], self.init_ball_pos[1]['y']),
                       self.init_ball_pos[1]['radius'], 0, -1)
            cv2.circle(img_binary,
                       (self.init_ball_pos[1]['x'], self.init_ball_pos[1]['y']),
                       self.init_ball_pos[1]['radius'], 0, 3)

            cv2.circle(img_binary,
                       (self.init_ball_pos[2]['x'], self.init_ball_pos[2]['y']),
                       self.init_ball_pos[2]['radius'], 0, -1)
            cv2.circle(img_binary,
                       (self.init_ball_pos[2]['x'], self.init_ball_pos[2]['y']),
                       self.init_ball_pos[2]['radius'], 0, 3)
    ...
    def check_first_ball(self, ball_path_list):
        if len(ball_path_list['red'])>=START_TRACKING_THRES and len(self.ball_path_list['yellow'])>=START_TRACKING_THRES and len(self.ball_path_list['white'])>=START_TRACKING_THRES:
            if self.valocity['red'] >= START_VALOCITY_THRES:
                self.first_ball = {'color': 'red', 'position':ball_path_list['red'][-1]}
                self.score = False
                self.event[self.event['event_num']] = {'name': 'shoot', 'discribe': '첫번째 공 빨간색', 'pos': self.ball_path_list['red'][-1], 'time': self.time}
                if args.print_log:
                    print("타구: ", self.event[self.event['event_num']]['discribe'])
                self.event['event_num'] += 1
            elif self.valocity['yellow'] >= START_VALOCITY_THRES:
    ...
    def simulation_based_event(self, time):
        self.time = time
        self.calc_distance()
        self.calc_valocity()
        ...
        #print(self.dist_white_yellow, self.ball_path_list['white'][-1])
        # 수구가 무엇인지 알아보자
        if self.first_ball == None:
            self.check_first_ball(self.ball_path_list)
            if self.first_ball != None:
        ...
        if self.tmp != None:
            if (time-self.tmp)==2:
                x_value = (self.ball_path_list[self.first_ball['color']][-1][0] -
                           self.ball_path_list[self.first_ball['color']][-2][0]) / 1000 * 2844
                y_value = (self.ball_path_list[self.first_ball['color']][-1][1] -
                           self.ball_path_list[self.first_ball['color']][-2][1]) / 500 * 1422
                self.direction = [x_value / max(abs(x_value), abs(y_value)), y_value / max(abs(x_value), abs(y_value))]

        # 수구를 쳤다면 1목적구가 무엇인지 알아보자
        if self.first_ball != None and self.second_ball == None:
            self.check_second_ball(BALL_CRASH_THRES_B, ACT_VALOCITY_THRES_S)
            if self.second_ball != None:
                self.check_ball_left_right()
                self.find_cuepoint()
                return 'object1'
        if self.first_ball != None and self.second_ball != None and self.third_ball == None:
            self.check_third_ball(BALL_CRASH_THRES_B, ACT_VALOCITY_THRES_S)
        elif self.third_ball != None:
            self.check_result()
            return 'object2'
        # 쿠션을 쳤는지 확인하자
    ...
    def path_analysis(self):
        ...
        while (1):
            ...
            if os.path.isfile(communication_path + 'unity2python.json'):
                time.sleep(0.1)
                with open(str(communication_path + 'unity2python.json'), 'r') as f:
                    sim_info = json.dumps(json.load(f))
                #print(sim_info.horizonHitPointPos)
                print("[유니티 파일 읽음]: ", communication_path + 'unity2python.json')
                # os.remove(communication_path + 'unity2python.json')
                break

        if cussion_num<2 or (cussion_num<2 and self.kiss == 'object1') or error=='error':
            self.event = {"event_num":0}
            self.first_ball = None
            self.second_ball = None
            self.third_ball = None
            ...
            if args.print_log:
                print("[정보 부족으로 인한 시뮬레이션 시작]")

            red, yellow, white = self.read_simulation_result(communication_path + 'unity2python.json')
            self.ball_path_list={'red':[],'yellow':[],'white':[]}
            for i in range(START_TRACKING_THRES, len(red)):
                self.ball_path_list['red']=red[:i]
                self.ball_path_list['yellow']=yellow[:i]
                self.ball_path_list['white']=white[:i]
    ...
    def expand_path(self):
        ...
        move_vec = move_vec/abs(min(move_vec))
        ...
        new_y = int(new_x * (move_vec[1] / move_vec[0]) + end[1] - (
                        move_vec[1] / move_vec[0]) * end[0])
    ...
    def write_json(self, send_data):
        ...
        data["init"]={"rPosInit": (np.array(self.ball_path_list["red"])[0] / 1000*2844).tolist(), "yPosInit": (np.array(self.ball_path_list["yellow"])[0] / 1000*2844).tolist(), "wPosInit": (np.array(self.ball_path_list["white"])[0] / 1000*2844).tolist(), "startingBall": send_data["start"], "object1": send_data["object1"],
                      "shootPoint": send_data['shoot'], "shootDirection": self.direction, "shootValocity": send_data['shoot_velocity'], "displayobject2":displayobject2}
        data["startingBallPos"]=[]
        action=0
        for event in self.event:
            try:
                int(event)
                data["startingBallPos"].append({"action":self.event[event]["name"], "pos":(np.array(self.event[event]["pos"])/1000*2844).tolist()})
                action=action+1
            except:
                continue

        data["allBallPath"]={}
        data["allBallPath"]["rBallPath"]=[]
        data["allBallPath"]["yBallPath"]=[]
        data["allBallPath"]["wBallPath"]=[]
        for pos in (np.array(self.ball_path_list["red"])[:] / 1000*2844).tolist():
            data["allBallPath"]["rBallPath"].append({"pos":pos})
        for pos in (np.array(self.ball_path_list["yellow"])[:] / 1000*2844).tolist():
            data["allBallPath"]["yBallPath"].append({"pos":pos})
        for pos in (np.array(self.ball_path_list["white"])[:] / 1000*2844).tolist():
            data["allBallPath"]["wBallPath"].append({"pos":pos})

        #print(len((np.array(self.ball_path_list["red"])[shoot_t:] / 1000*2844).tolist()), len((np.array(self.ball_path_list["yellow"])[shoot_t:] / 1000*2844).tolist()), len((np.array(self.ball_path_list["white"])[shoot_t:] / 1000*2844).tolist()))

        with open('./../communication/P2U.json', 'w', encoding='utf-8') as make_file:
            json.dump(data, make_file, indent="\t")
        os.rename('./../communication/P2U.json', './../communication/Python2Unity.json')
...
'''def find_ball(_, show_img, img_color, __):
    circles=None
    mean = np.median(img_color, axis=(0, 1))
    ...
    if circles is not None:
        ...
        if len(ball_list)==3:
            ...
            #if len(ball_list)==3:
            ball_list = [ball_list[red_idx], ball_list[yellow_idx], ball_list[white_idx]]

            sorted_ball_list={}
            sorted_ball_list['red'] = ball_list[0]
            cv2.circle(show_img, (sorted_ball_list['red']['x'], sorted_ball_list['red']['y']), sorted_ball_list['red']['radius'], (0, 0, 1), 2)
            sorted_ball_list['yellow'] = ball_list[1]
            cv2.circle(show_img, (sorted_ball_list['yellow']['x'], sorted_ball_list['yellow']['y']), sorted_ball_list['yellow']['radius'], (0, 1, 1), 2)
            sorted_ball_list['white'] = ball_list[2]
            cv2.circle(show_img, (sorted_ball_list['white']['x'], sorted_ball_list['white']['y']), sorted_ball_list['white']['radius'], (1, 1, 1), 2)

            return show_img, sorted_ball_list, radius
    return show_img, None, None'''

def find_ball(img, show_img, img_color, event_checker):
    radius = None

    COLORS = [(1, 1, 1), (0, 0, 1), (0, 1, 1)]
    # img_color=img_color*0.8
    # img_color=img_color/255
    # cv2.imshow("asdf", img_color/255)
...
def predict_func(path):
    ...
    for i in args.input.split('/')[:-1]:
        directory += i
        directory += '/'
    vid_path = 'visualization/' + name + '.mp4'
    vid_writer = cv2.VideoWriter(vid_path, cv2.VideoWriter_fourcc(*'mp4v'), 30, (WIDTH, HEIGHT))
    ...
    for t, datas in enumerate(imgs):
        ...
        img_original, img_color, _ = datas
        ...
        vid_writer.write(np.uint8(show_img))
        cv2.waitKey(1)

    # event_checker.remove_lf()
    if event_checker==None:
        print("ERROR: can't find any ball")
        return
    event_checker.write_json(send_data)
    ...
    os.remove('./../communication/' + 'unity2python.json')
    vid_writer.release()
...
def file_move(input_path):
    os.rename('./input_file/' + input_path, './processing_file/'+input_path.split('/')[-1])
    return str('./processing_file/'+input_path.split('/')[-1])

if __name__ == '__main__':

    print("[Program Start]")
    path_list = os.listdir('../videos')
    for path in path_list:
        predict_func('../videos/'+path)
